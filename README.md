...
a bounded blocked-request count, and whether an expected visible effect was
prevented. Sanitized diagnostics never retain request URLs, bodies, headers,
credentials, or tokens. This classification is the WES-269 foundation for
WES-266; `public-browse` is not implemented yet.
...
- `@auto-demo/agent`: agent-facing workflow helpers, JSON handoff summaries, evidence-backed walkthrough compilation, approval-aware recorded execution, the versioned discovery-session evidence contract, and repository-owned Codex-hosted risk-tiered discovery and approval artifacts. Claude production wrapper remains follow-up scope.
...
the compiled plan artifact, replay and repair it in a fresh context, then use the
shared plan-file review, refine, approve, execute, and handoff commands.
