...
The command starts an HTTP server on `127.0.0.1` and an OS-assigned port by default, then prints the local URL. The editor shell loads the project through the same validation path as `@auto-demo/project`, reports stable operator-readable validation errors, and lists saved/generated variants from the project manifest. Projects with no saved variants show a guidance message pointing back to `autodemo generate --project <project> --json --save all`. The first editor slice is review-only; timeline preview controls, editing, export rendering, and browser auto-launch are planned follow-up work.
