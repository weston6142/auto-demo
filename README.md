...
The command starts an HTTP server on `127.0.0.1` and an OS-assigned port by default, then prints the local URL. Use `--host` and `--port` to override the bind address; `--port 0` keeps OS port assignment. `--no-browser` is accepted as a no-op compatibility flag because WES-157 never auto-launches a browser. The editor shell loads the project through the same validation path as `@auto-demo/project`, reports stable operator-readable validation errors, and lists saved/generated variants from the project manifest. Projects with no saved variants show a guidance message pointing back to `autodemo generate --project <project> --json --save all`. WES-170 chose approximate browser review fidelity for the next editor slice: WES-158 should add schema-backed trim, viewport, caption/callout, cursor/click, and direct style controls while deferring exact export parity, named preset picker UI, export rendering, and browser auto-launch.
