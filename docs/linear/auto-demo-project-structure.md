...
Prefer the earliest milestone with incomplete issues. Within that milestone, prefer started issues, then unblocked design/spec issues, then implementation issues whose dependencies are satisfied. Current next product task after WES-163 completion: WES-168, because the packaging target decision should inform WES-164 before local install/package entrypoint implementation and WES-165 remains blocked by WES-164.
